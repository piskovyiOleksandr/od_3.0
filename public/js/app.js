...
function mrzv_popup_ajax(obj, data) {
	...
	$('.mrzv-overlay').animate({ opacity: '.7' }, 300);
...
function constriction_chat(type) {
	if (type == 'thin' && $('body').width() > 850)
		$('body').animate({ width: $('body').width() - 360 + 'px' }, 300, 'swing')
...
// $('.universal-input').on('click', (e) => {
// 	$('.universal-input-wrap').removeClass('focus')
// 	let i = $(e.target)[0]
// 	if ($(i).is(":focus")) {
...
$('.settings-toggle input[type="checkbox"]').click(function () {
	$(this).parent().toggleClass('on');

});
// Sale timer
let seconds = 59 * 60 + 1, h, m, s, t;
function countDown() {
	if (seconds > 0) {
		seconds--;
		h = seconds / 3600 ^ 0,
...
$('.btn-save-profile').click(() => {
	...
	userPofile = {
		...
		// 'user-gender': $('.user-gender')[0].data('name'),
		'user-age': $($('.user-age')[0]).val(),
		'user-language': $($('.user-language')[0]).val(),
	}
